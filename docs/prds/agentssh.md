...
AgentSSH 是一个本地运行的「最小权限 SSH 代理」:AI Agent 可参考外部操作手册,但只能通过 `agentssh` CLI 执行;凭据与动作边界由人类掌控,每次操作分级审批、全量审计。**服务器侧零安装。**
...
- **操作手册 / skill**:可用 Anthropic Agent Skills 形式的 `SKILL.md` + 配套文件表达。人类编写的*程序性知识*,教 Agent「怎么用好 AgentSSH」—— 信任边界、会话纪律、策略、输出克制、审计复盘,而非某台服务器上某个应用的具体操作流程。它是外部知识,不是 AgentSSH 的 RPC、权限或审计主体。属于「软控制」—— 影响 Agent 会尝试什么,但不负责安全。
- **CLI(`agentssh`)**:Agent 实际调用的工具,也是**信任边界**。握凭据、跑 policy、做审批、写审计。属于「硬控制」—— 安全只依赖这一层。
- **Inventory**:人类维护的主机/分组清单(及其连接方式、凭据引用)。
- **Policy**:人类配置的规则,决定一条命令是 `allow` 执行或 `deny` 拒绝;AgentSSH 自身不提供“需审批”中间态。
...
- G3 提供**不可篡改的全量审计**,人类可事后还原每一次「命令 + 目标 + 决策 + 结果」。
...
- **FR3 Policy(allow/deny)**:每条命令被判定为 `allow`(执行)或 `deny`(硬拦截、不可临场覆盖、headless 亦生效);未匹配任何规则一律默认 `deny`。规则带 `action` 与 `priority`,先判定 host tier(`host:<name>` 与匹配 group 的规则合并),再判定 global tier;host tier 永远高于 global tier。
- **FR4 交互确认(委托)**:AgentSSH 不做同步审批、不阻塞等待人工裁决;灰区命令的交互式 allow/deny 由 agent harness(如 Claude Code 权限系统)负责。
- **FR5 审计**:每次请求(含命令、目标、决策、退出码、输出摘要/hash)写入 hash 链日志;提供查询与校验。
- **FR6 输出控制**:回流给 Agent 的输出按规则脱敏、按上限截断。
- **FR7 能力发现**:Agent 可 `agentssh hosts` 列主机;外部操作手册由 Agent 自身的 skill 机制发现。
- **FR8 请求查询**:Agent 可 `agentssh status <req>` 从审计中查询某次执行的结果(退出码/是否被 deny)。
- **FR9 会话分组**:相关的一连串 `run` 归入同一 `session_id`,由调用方显式声明(`--session` 或 `$AGENTSSH_SESSION`;`agentssh session new` 可 mint 一个 id)。一个任务一个会话;未声明则 `run` 报错(exit 2),系统不按时间窗口自动归并,以免不同任务串台。审计可按会话浏览与复盘,会话可带 `session_label`。

## 8. 关键用户故事

- **U1(排障)**:nginx 在 web-1 返 502。人类先为本次允许的诊断/修复命令配置 allow 规则。Agent 按 `agentssh-usage` 使用手册的最佳实践:`agentssh hosts` 确认目标 → `export AGENTSSH_SESSION=$(agentssh session new)` 为本次任务开一个会话 → 只读诊断 `run web-1 -- systemctl status nginx` → 发现挂了 → 改动前先小结 → `run web-1 -- sudo systemctl restart nginx`;命中 allow 后执行成功 → 全程入同一会话的审计。人类事后用 `agentssh tui` 复盘。
- **U2(审计回溯)**:人类周一早上 `agentssh tui` 按**会话**(每个任务一组)浏览周末 Agent 的全部操作,展开可疑会话细看,确认没有越界,并校验审计链完整。
- **U3(收紧策略)**:人类为 `prod` 组添加高优先级 deny 规则与必要 allow 规则;未命中的命令天然 default-deny,无需再把某组“切换”为 allowlist 模式。
...
- Policy 默认拒绝 + `deny` 硬拦截 + 全量审计 —— **无同步审批**,交互确认交给 harness
...
> 进展:**native SSH 已实现 Phase 1 + Phase 2a**:Phase 1 = opt-in 原生后端(`docs/architecture/native-ssh.md`,commit 10a8b94);Phase 2a = 流式输出(`docs/architecture/streaming-output.md`,commit e1cb39d)。**仅剩 Phase 2b 连接复用(ControlMaster,需常驻 daemon)未做**。其余增强项(out-of-band 审批、Web、多人、MCP server、凭据 vault、事务回滚、远端 agent)均未开始。
