...
- **操作手册 / playbook**:可用 Anthropic Agent Skills 形式的 `SKILL.md` + 配套文件表达。人类编写的*程序性知识*,教 Agent「某类运维任务怎么做」。它是外部知识,不是 AgentSSH 的 RPC、权限或审计主体。属于「软控制」—— 影响 Agent 会尝试什么,但不负责安全。
...
- **FR9 会话分组**:相关的一连串 `run` 归入同一 `session_id`(显式 `--session` / `$AGENTSSH_SESSION` / 空闲 30m 的时间窗口自动);审计可按会话浏览与复盘,会话可带 `session_label`。
